...
def bo_expected_improvement_search(embedding_path, total_rounds, save_path, init_size, k, seed):
    ...
    while counter <= MAX_BUDGET:
        ...
        model = Bohamiann(print_every_n_steps=1000, normalize_output=False)
        model.train(x_train=feat_samples.numpy(), y_train=valid_fom_samples.view(-1).numpy(), num_steps=5000, num_burn_in_steps=2000, keep_every=50, lr=1e-2, batch_size=10)
