...
class DVAE_PYG(nn.Module):
    def __init__(self, max_n, nvt, START_TYPE, END_TYPE, hs=501, nz=56, bidirectional=False, vid=True, num_layers=1, topo_feat_scale=0.01):
        ...
        # 0. encoding-related
        self.grue_forward = nn.ModuleList([nn.GRUCell(nvt + 1, hs) if l == 0 else nn.GRUCell(hs, hs) for l in range(num_layers)])  # encoder GRU
        self.grue_backward = nn.ModuleList([nn.GRUCell(nvt + 1, hs) if l == 0 else nn.GRUCell(hs, hs) for l in range(num_layers)]) # backward encoder GRU
        self.fc1 = nn.Linear(self.gs, nz)  # latent mean
        self.fc2 = nn.Linear(self.gs, nz)  # latent logvar
            
        # 1. decoding-related
        self.grud = nn.ModuleList([nn.GRUCell(nvt + 1, hs) if l == 0 else nn.GRUCell(hs, hs) for l in range(num_layers)]) # decoder GRU  # TODO we here leave one layer?
    ...
    def loss(self, mu, logvar, G_true, beta=0.005):
        ...
        for v_true in range(1, self.max_n):
            ...
            true_types = [torch.argmax(g_true.x[v_true,:-1]).item() if v_true < g_true.x.shape[0]
                          else self.START_TYPE for g_true in G_true]
            true_feats = [g_true.x[v_true, -1] if v_true < g_true.x.shape[0]  # (bsize, 1)
            ...
            vl2  = self.topo_feat_scale * F.mse_loss(pred_feats, true_feads, reduction='sum')/100
            res = res + vll - vl2
            res_vll += vll
            res_vl2 += vl2
            for i, g in enumerate(G):
            ...
            for i, g_true in enumerate(G_true):
                t_idx = g_true.edge_index[0] == v_true
                s_idx = g_true.edge_index[1][t_idx]
